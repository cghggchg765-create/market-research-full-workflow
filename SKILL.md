...
3. **生成完整 Writer 上下文注入包（强制，缺一不可）**：编排者运行
   `	ext
   {python} {skill_root}/scripts/report/writer_context.py --run-dir {run_dir}
   `
   为 8 个分片各生成 evidence/writer-context.{NN}.md，每个注入包自动汇总：全局判断主线全文、全局大纲全文、完整图表清单（含每张图所属章节）、全文档表号预分配、其他 7 章核心论点摘要、本章证据注入包、本章图号/表号区间、写作纪律。**编排者必须将对应注入包全文原文粘贴进 Writer prompt，不得只给文件路径或删减内容。**
4. **每个 Writer 必须收到完整上下文（缺一不可，原文粘贴而非只给路径）**：
   - **全局判断主线**：`analysis/judgment-spine.md` 全文（含一句话主线、三情景、T 信号、各板块子命题）
   - **全局大纲**：`analysis/framework.md` 全文（含所有章节的小节标题、核心论点、预分配图表/表格位置）
   - **完整图表清单**：`charts/chart-manifest.json` 全量条目（每张图的图号、标题、所属章节 position、数据来源），Writer 只能引用分配给本章的图号，**严禁引用其他章节的图号或自行新增图号**
   - **表号预分配表**：编排者在规划阶段生成的全文档表号清单（表1…表N，每张表的所属章节和表名），Writer 只能使用分配给本章的表号
   - **其他章节摘要**：除本章外，其余 7 个分片的**核心论点摘要**（每章 3-5 句，由编排者从 framework.md 提炼或从前序已完成分片中提取），防止内容重复、论点冲突、数据不一致
   - **本章证据注入包**：`evidence/cards.{NN}.md` 卡片片段 + `evidence/inject.{NN}.json`
   - **本章关键数据点**：注入包中每张卡片的「关键数据点 + 解读策略 + 使用建议 + 反证与边界」原文直接粘贴
   - **本章图号/表号区间**：明确告知"本章只能使用图X-图Y、表A-表B"，超出范围即视为错误
   Writer 必须把至少 80% 的论点建立在粘贴的数据点上并写入正文数值；**每个引用的数据点须配一句解读**；写作前先对照"其他章节摘要"确认本章不重复已有论述；缺证据写入待核问题，不得自行编造图号/表号。
5. **单片即时校验（写作完成即执行）**：每个 Writer 落盘后，编排者立即运行
   ```text
...
6. 编排者检查 8 片 UTF-8/文件名/mtime/字数/章节职责，并核对 `charts/chart-manifest.json` 图号与规划表预分配一致；任一未就绪不得进入组装。
